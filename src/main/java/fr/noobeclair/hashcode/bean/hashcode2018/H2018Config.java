...
public class H2018Config extends Config {
    ...
    private static final List<Integer> CSV_CST = Arrays.asList(StatsConstants.CF_STRAT, StatsConstants.SCORE, StatsConstants.TIME_TOTAL, StatsConstants.ITEM0_TOTAL,
                    StatsConstants.IN_0,
                    StatsConstants.IN_1,
                    StatsConstants.CF_TTFC, StatsConstants.CF_NTFCT, StatsConstants.CF_NDFCT, StatsConstants.CF_LTFCT, StatsConstants.CF_LDFCT, StatsConstants.CF_NAT,
                    StatsConstants.CF_NBT,
    ...
    private final Double timeToFinishCoef;

    @ConfGenerable(type = TYPE.ENUM, eClass = AdjustMethod.class, excludes = {"LN", "POW"})
    private final AdjustMethod nearTravelAdjustFct;
    // private AdjustMethod longTravelAdjustFct;
    // @ConfGenerable(type = TYPE.ENUM, eClass = AdjustMethod.class, excludes = { "LN", "POW" })
    private final AdjustMethod nearDistAdjustFct;
    // private AdjustMethod longDistAdjustFct;

    @ConfGenerable(type = TYPE.DOUBLE, min = "1", max = "16", step = "5")
    private final Double nearATravelMethodCst;
    // @ConfGenerable(type = TYPE.DOUBLE, min = "0", max = "10", step = "5")
    private final Double nearBTravelMethodCst;
    // @ConfGenerable(type = TYPE.DOUBLE, min = "1", max = "16", step = "5")
    private final Double nearADistMethodCst;
    // @ConfGenerable(type = TYPE.DOUBLE, min = "0", max = "10", step = "5")
    private final Double nearBDistMethodCst;
    ...
    private final List<CarStrategy> carStrategies;
    ...
    public H2018Config(final Double coefBonus, final Double nearBonus, final Double distanceBonus) {
        super(CSV_CST, CSV_PATH);
        this.timeToFinishCoef = 1D;
        this.nearTravelAdjustFct = AdjustMethod.EXP;
        this.nearATravelMethodCst = 1D;
    ...
    public H2018Config(final Double timeToFinishCoef, final AdjustMethod nearTravelAdjustFct, final AdjustMethod nearDistAdjustFct, final AdjustMethod longTravelAdjustFct,
                       final AdjustMethod longDistAdjustFct,
                       final Double nearATravelMethodCst, final Double nearBTravelMethodCst, final Double nearADistMethodCst, final Double nearBDistMethodCst,
                       final Double longATravelMethodCst, final Double longBTravelMethodCst,
                       final Double longADistMethodCst, final Double longBDistMethodCst, final List<CarStrategy> carStrategies) {
        super(CSV_CST, CSV_PATH);
    ...
    public Double adjustNearTravel(final Double t) {
    ...
    public Double adjustNearDist(final Double t) {
        if (CollectionUtils.containsAny(carStrategies, Arrays.asList(CarStrategy.AGGRESSIVE, CarStrategy.NEAR_FIRST))) {
    ...
    public Double adjustLongTravel(final Double t) {
        if (CollectionUtils.containsAny(carStrategies, Arrays.asList(CarStrategy.AGGRESSIVE, CarStrategy.NEAR_FIRST))) {
            switch (nearTravelAdjustFct) {
            case EXP:
                return Math.exp(t + nearBTravelMethodCst);
    ...
    public Double adjustLongDistance(final Double t) {
    ...
    public Double adjustTimeWhenFinishRatio(final Double t, final Double d, final Integer turn, final Integer maxturn) {
        Double ratio = 0D;
        if (CollectionUtils.containsAny(carStrategies, Arrays.asList(CarStrategy.AGGRESSIVE, CarStrategy.LONG_FIRST))) {
            final Double diff = maxturn - (t + d + turn);
            if (diff > 0) {
