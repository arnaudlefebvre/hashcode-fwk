...
public class AlgoUtils<T extends Bean> {
     ...
     */
    public DistanceResultDto<T> nearestSibling(final T ref, final List<T> list) {
        ...
        T tmp = null;
        for (final T d : list) {
            final double cur = ref.distance(d);
            if (max > cur) {
                tmp = d;
                max = cur;
                resIdx = idx;
     ...
     * @return nearest object of ref in list
     */
    public DistanceResultDto<T> nearestSibling(final T ref, final List<T> list, final HashMap<Integer, T> exclude) {
        final long start = System.currentTimeMillis();
        logger.trace("-- nearestSibling start");
        double max = Double.MAX_VALUE;
        int idx = 0, resIdx = 0;
        ...
        for (final T d : list) {
            ...
            final double cur = ref.distance(d);
            // logger.debug("nearestSibling ref {}, max dist {}, current {} (hash : {}) from distance {}, processed
            // {}",ref,max,d,d.hashCode(),cur,exclude);
     ...
     */
    public DistanceResultDto<T> farthestSibling(final T ref, final List<T> list) {
        final long start = System.currentTimeMillis();
     ...
     *            list
     * @param exclude
     *            {@link HashMap<Integer, T > } exclude these items from
     *            comparison
     * @return farthest object of ref in list
     */
    public DistanceResultDto<T> farthestSibling(final T ref, final List<T> list, final HashMap<Integer, T> exclude) {
        final long start = System.currentTimeMillis();
        logger.trace("-- farthestSibling start");
        double max = 0;
        int idx = 0, resIdx = 0;
