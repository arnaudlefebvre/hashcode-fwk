	...
	public static String formatToHHMMSS(Long sec) {
		...
		if (sec < maxh) {
			return String.format("%02dh%02dm%02ds", TimeUnit.SECONDS.toHours(sec),
					TimeUnit.SECONDS.toMinutes(sec) - TimeUnit.HOURS.toMinutes(TimeUnit.SECONDS.toHours(sec)), // The
																												// change
																												// is in
																												// this
																												// line
			...
			return String.format("%02dd%02dh%02dm", TimeUnit.SECONDS.toDays(sec),
					TimeUnit.SECONDS.toHours(sec) - TimeUnit.DAYS.toHours(TimeUnit.SECONDS.toDays(sec)), // The change
																											// is in
																											// this line
