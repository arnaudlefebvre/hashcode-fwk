...
import fr.noobeclair.hashcode.bean.custom.CustomWorkerConfig;
...
import fr.noobeclair.hashcode.bean.hashcode2018.H2018WorkerConfig;
import fr.noobeclair.hashcode.bean.hashcode2019.H2019Config;
import fr.noobeclair.hashcode.bean.hashcode2019.H2019WorkerConfig;
...
/**
 * Main gérant le lancement de l'algo sur les fichiers donnés et générant les fichiers de résultats
 */
public class MainRunner {
	...
	public static void main(final String[] args) {
		...
		try {
			...
			// Create solver config - with your default var that can change for each solver runs
			CustomWorkerConfig wcfg = new CustomWorkerConfig();
			// Create solver config - with your default var that can change for each solver runs
			...
			SimpleConfWorker<CustomBeanContainer, CustomConfig> cfgsw = new SimpleConfWorker<>(creader, cfgsolver,
					cscorecalc, cwriter, io);
			// 3 Multiple config : n files, n workers
			MultipleConfFileSolverWorker<CustomBeanContainer, CustomConfig, CustomConfigSolver, CustomWorkerConfig> mw = new MultipleConfFileSolverWorker<CustomBeanContainer, CustomConfig, CustomConfigSolver, CustomWorkerConfig>(
					creader, cwriter, cscorecalc, 0, cfg, wcfg);
			...
			H2019WorkerConfig wcfg2019 = new H2019WorkerConfig();
			...
			final HashCode2019DummyStepSolver dummySolver = new HashCode2019DummyStepSolver("Dummy2019", timeout,
					cfg2019);
			...
			final SimpleConfWorker<HashCode2019BeanContainer, H2019Config> sw = new SimpleConfWorker<>(reader,
					dummySolver, scorer, writer, new InOut("src/main/resources/in/b_lovely_landscapes.txt",
			...
			final MultipleConfFileWorker<HashCode2019BeanContainer, H2019Config, HashCode2019StepSolver, H2019WorkerConfig> mfw = new MultipleConfFileWorker<>(
					reader, writer, scorer, solver, cfg2019, wcfg2019);
			...
			final MultipleConfSolverWorker<HashCode2019BeanContainer, H2019Config, HashCode2019StepSolver, H2019WorkerConfig> msw = new MultipleConfSolverWorker<>(
					reader, writer, scorer,
			...
			final MultipleConfFileSolverWorker<HashCode2019BeanContainer, H2019Config, HashCode2019StepSolver, H2019WorkerConfig> mfsw = new MultipleConfFileSolverWorker<>(
					reader, writer, scorer, Arrays.asList(solver), cfg2019, wcfg2019);
			mfsw.addFiles(files);
			// scores = mfsw.run();
			// if (MapUtils.isNotEmpty(scores)) {
			...
			H2018Writer null2018 = new H2018Writer(false);

			H2018WorkerConfig workerCfg = new H2018WorkerConfig();
			H2018Config solverCfg = new H2018Config(0.0, 0.0, 0.0);
			...
			List<H2018Config> configs2018 = config2018factory.generate(solverCfg);
			SolverFactory<H2018Solver, H2018BeanContainer, H2018Config> sfactory = new SolverFactory<>(
					H2018Solver.class, H2018BeanContainer.class, H2018Config.class);
			WorkerFactory<H2018BeanContainer, H2018Reader, H2018Writer, H2018Config, H2018Solver, H2018ScoreCalculator, H2018WorkerConfig> sw0 = new WorkerFactory<H2018BeanContainer, H2018Reader, H2018Writer, H2018Config, H2018Solver, H2018ScoreCalculator, H2018WorkerConfig>();
			MultipleConfFileSolverWorker<H2018BeanContainer, H2018Config, H2018Solver, H2018WorkerConfig> mfsw2018 = sw0
					.builder(H2018WorkerConfig.class).readWrite(read2018, null2018).score(scor2018)
					.solverConfig(solverCfg).solvers(sfactory.createFromConfs(configs2018, timeout)).progressBar()
					...
					.file(new InOut("src/main/resources/in/2018/e_high_bonus.in", null)).Build();
			scores = mfsw2018.run();
			...
			List<H18LegConf.CarStrategy> carSt3 = Arrays.asList(H18LegConf.CarStrategy.NEAR_FIRST,
					H18LegConf.CarStrategy.QUICK_FIRST);
			List<H18LegConf.CarStrategy> carSt4 = Arrays.asList(H18LegConf.CarStrategy.LONG_FIRST);
			List<H18LegConf.CarStrategy> carSt5 = Arrays.asList(H18LegConf.CarStrategy.LONG_FIRST,
					H18LegConf.CarStrategy.QUICK_FIRST);
			...
			configs.add(new H18LegConf(2.0, H18LegConf.AdjustMethod.EXP, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.1, 0.0, 1.5, 0.0, 1.5, 0.0,
					carSt2));
			configs.add(new H18LegConf(2.0, H18LegConf.AdjustMethod.EXP, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.1, 0.0, 1.5, 0.0, 1.5, 0.0,
					carSt3));
			...
			//
			// // Winners Custom Try
			configs.add(new H18LegConf(3.0, H18LegConf.AdjustMethod.LINEAR, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.1, 0.0, 1.5, 0.0, 1.5, 0.0,
					carSt4));
			configs.add(new H18LegConf(1.0, H18LegConf.AdjustMethod.LINEAR, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.1, 0.0, 1.3, 0.0, 1.5, 0.0,
					carSt4));
			configs.add(new H18LegConf(1.0, H18LegConf.AdjustMethod.LINEAR, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.0, 0.0, 1.1, 0.0, 1.3, 0.0,
					carSt4));
			configs.add(new H18LegConf(4.0, H18LegConf.AdjustMethod.EXP, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.1, 0.0, 1.5, 0.0, 1.5, 0.0,
					carSt3));
			configs.add(new H18LegConf(2.0, H18LegConf.AdjustMethod.EXP, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 2.0, 0.0, 1.8, 0.0, 1.5, 0.0, 1.5, 0.0,
					carSt3));
			configs.add(new H18LegConf(2.0, H18LegConf.AdjustMethod.EXP, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.1, 0.0, 2.0, 0.0, 2.0, 0.0,
					carSt4));
			configs.add(new H18LegConf(2.0, H18LegConf.AdjustMethod.EXP, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.1, 0.0, 1.8, 0.0, 1.8, 0.0,
					carSt3));
			configs.add(new H18LegConf(2.0, H18LegConf.AdjustMethod.EXP, H18LegConf.AdjustMethod.LINEAR,
					H18LegConf.AdjustMethod.INV, H18LegConf.AdjustMethod.LINEAR, 1.0, 0.0, 1.5, 0.0, 1.5, 0.0, 1.5, 0.0,
					carSt3));
			// logger.error("Count EXP {}", configs.stream().filter(c -> c.getNearTravelAdjustFct().equals(H18LegConf.AdjustMethod.EXP)).count());
			logger.error("Count LINEAR {}", configs.stream()
					.filter(c -> c.getNearTravelAdjustFct().equals(H18LegConf.AdjustMethod.LINEAR)).count());
			logger.error("Count INV {}", configs.stream()
					.filter(c -> c.getNearTravelAdjustFct().equals(H18LegConf.AdjustMethod.INV)).count());
			H18LegConf wcfgOld = new H18LegConf(0.0, 0.0, 0.0);
			SolverFactory<H2018SolverLegConf, H2018BeanContainer, H18LegConf> sfactoryOld = new SolverFactory<>(
					H2018SolverLegConf.class, H2018BeanContainer.class, H18LegConf.class);
			WorkerFactory<H2018BeanContainer, H2018Reader, H2018Writer, H18LegConf, H2018SolverLegConf, H2018ScoreCalculator, H2018WorkerConfig> sw0Old = new WorkerFactory<H2018BeanContainer, H2018Reader, H2018Writer, H18LegConf, H2018SolverLegConf, H2018ScoreCalculator, H2018WorkerConfig>();
			MultipleConfFileSolverWorker<H2018BeanContainer, H18LegConf, H2018SolverLegConf, H2018WorkerConfig> mfsw2018Old = sw0Old
					.builder(H2018WorkerConfig.class).readWrite(read2018, null2018).score(scor2018)
					.solverConfig(wcfgOld).solvers(sfactoryOld.createFromConfs(configs, timeout)).progressBar()
					.file(new InOut("src/main/resources/in/2018/a_example.in", null))
					.file(new InOut("src/main/resources/in/2018/b_should_be_easy.in", null))
					.file(new InOut("src/main/resources/in/2018/c_no_hurry.in", null))
					.file(new InOut("src/main/resources/in/2018/d_metropolis.in", null))
			...
			//
			// final MultipleConfFileSolverWorker<H2018BeanContainer, H2018Config, H2018Solver> mfsw2018 = new MultipleConfFileSolverWorker<>(read2018, nwriter,
			// scor2018, 1, workerCfg);
