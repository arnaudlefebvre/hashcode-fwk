...
public class MultipleConfSolverWorker<T extends BeanContainer, V extends Config, S extends ConfigSolver<T, V>, W extends WorkerConfig>
		extends MultipleConfWorker<T, V, S, W> {

	public MultipleConfSolverWorker(final InReader<T> reader, final OutWriter<T> writer,
			final ScoreCalculator<T> scorer, final String in, final String out, V config, W wconfig) {
	...
	public MultipleConfSolverWorker(final InReader<T> reader, final OutWriter<T> writer,
			final ScoreCalculator<T> scorer, final InOut inOut, V config, W wconfig) {
		super(writer, reader, scorer, new ArrayList<>(), config, wconfig);
		this.files = new ArrayList<>();
	...
	public void addSolvers(final List<S> solvers) {
		for (S s : solvers) {
			this.solvers.add(s);
		}
	}
